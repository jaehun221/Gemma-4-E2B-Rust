...
pub fn apply_rope(q: &mut Array2<f32>, cos_table: &Array2<f32>, sin_table: &Array2<f32>) {
    let half = q.dim().1 / 2;
...
pub fn mlp(
    ...
    up_proj: ArrayView2<f32>,
    down_proj: ArrayView2<f32>,
) -> Array2<f32> {
...
fn attention(
    ...
    for head_idx in 0..cfg.num_attention_heads {
        let q_head = q
            .slice(s![.., head_idx * head_dim..head_idx * head_dim + head_dim])
            .to_owned();
...
pub fn decoder_block(
    ...
    let h = mlp(
        h.view(),
        block.mlp.gate_proj.view(),
        block.mlp.up_proj.view(),
        block.mlp.down_proj.view(),
    );
    ...
    // Per Layer Embedding
    let residual = h.clone();
    let mut h = h.dot(&block.ple.input_gate.t());
    h = h.mapv(gelu);
    h = h * per_layer_input;
